...
class StockHubDriverActivity : AppCompatActivity() {
                ...
                imageRef.downloadUrl.addOnSuccessListener { uri ->
                    ...
                    db.collection("stock").document(deliveryId).set(deliveryData)
                        .addOnSuccessListener {
                            ...
                            navigateToConfirmation(driverId, deliveryLocation, quantity, uri.toString(), deliveryId)
                        }
    ...
    private fun navigateToConfirmation(driverId: String, deliveryLocation: String, quantity: Long, imageUrl: String, deliveryId: String) {
        Log.d(TAG, "Navigating to DeliveryConfirmationActivity with values: driverId=$driverId, deliveryLocation=$deliveryLocation, quantity=$quantity, imageUrl=$imageUrl")

        val intent = Intent(this, DeliveryConfirmationActivity::class.java).apply {
            putExtra("driverId", driverId)
            putExtra("deliveryId", deliveryId) // Pass the deliveryId
            putExtra("deliveryLocation", deliveryLocation)
            putExtra("quantity", quantity) // Pass quantity as Long
